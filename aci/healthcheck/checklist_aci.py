...
class ACIHealthChecker:
    ...
    def apic_login(
        ...
        try:
            ...
            if resp.status_code != 200:
                self.console.print(
                    f"[red]✗ Login failed with status code: {resp.status_code}[/red]"
                )
                return None
            ...
            if "imdata" in response_data and len(response_data["imdata"]) > 0:
                if (
                    isinstance(response_data["imdata"][0], dict)
                    and "error" in response_data["imdata"][0]
                ):
                    self.console.print(
                        "[red]✗ Authentication failed: Invalid credentials[/red]"
                    )
                    return None

            self.console.print(
                f"[green]✓ Successfully authenticated to APIC {apic_ip}[/green]"
            )
            return resp.cookies
    ...
    class APIClient:
        ...
        def fetch_api(
            self, url: str, description: str = "Fetching data"
        ) -> Optional[Dict]:
            """Generic API fetch function with error handling"""
            try:
                with self.console.status(
                    f"[cyan]{description}...[/cyan]", spinner="dots"
                ):
                    response = requests.get(
                        url, cookies=self.cookies, verify=False, timeout=60
                    )

                if response.status_code != 200:
                    self.console.print(
                        f"[yellow]⚠ API call to {url} returned status {response.status_code}[/yellow]"
                    )
                    return None
                ...
                self.console.print(
                    f"[yellow]⚠ Error while {description}: {str(e)}[/yellow]"
                )
                return None
    ...
    class DataProcessor:
        ...
        def _get_first_child_attributes(item: Dict, child_key: str) -> Dict:
            ...
            children = (
                item.get(list(item.keys())[0], {}).get("children", [])
        ...
        def process_apic_data(data: Dict) -> List[Dict]:
            ...
            if (
                ...
                and "infraWiNode" in data
                and isinstance(data["infraWiNode"], list)
            ):
                    ...
                    nodes.append(
                        {
                            ...
                            "health": 100
                            if str(attrs.get("health", "")).lower()
                            in ["fully-fit", "100"]
                            else 50
                            if str(attrs.get("health", "")).lower() == "degraded"
                            else int(attrs.get("health", 0) or 0),
                        }
                    )
                return nodes
            ...
            for entry in imdata:
                ...
                name = (
                    attrs.get("nodeName") or attrs.get("name") or attrs.get("id") or ""
                )
                serial = attrs.get("mbSn") or attrs.get("serial") or ""
                # ip may be stored in different fields
                ip = (
                    attrs.get("oobNetwork", {}).get("address4", "").split("/")[0]
                    if isinstance(attrs.get("oobNetwork"), dict)
                    else attrs.get("oobMgmtAddr", "") or attrs.get("address", "")
                )
                ...
                try:
                    ...
                    numeric_health = (
                        100
                        if str(health_str).lower() in ["fully-fit", "fully fit"]
                        else 50
                        if str(health_str).lower() == "degraded"
                        else 0
                    )
        ...
        @staticmethod
        def process_leaf_spine(
            top_data: Dict, cpu_data: Dict, mem_data: Dict
        ) -> List[Dict]:
                ...
                for m in mem_data.get("imdata", []):
                    ...
                    if "PercUsedMemoryAvg" in attrs:
                        ...
                        try:
                            ...
                            mem_val = (
                                (used_avg / total_avg) * 100 if total_avg > 0 else 0.0
                            )
                        except Exception:
                            mem_val = 0.0
            ...
            for item in top_data.get("imdata", []):
                ...
                try:
                    health_score = int(
                        health_attr.get("cur", attr.get("health", 0) or 0)
                    )
                except Exception:
                    try:
                    ...
                    if last_change_str:
                        try:
                            ...
                            last_change = datetime.strptime(
                                last_change_str.split(".")[0], "%Y-%m-%dT%H:%M:%S"
                            )
        ...
        def process_fabric_health(data: Dict) -> int:
            ...
            # fallback
            try:
                health_attr = data["imdata"][0][
                    next(iter(data["imdata"][0].keys()))
                ].get("attributes", {})
    ...
    class ReportGenerator:
        ...
        def print_report(
            ...
            health_status = (
                "Normal"
                if fabric_health >= self.health_threshold
        ...
        def _print_apic_table(self, apic_nodes: List[Dict]):
            ...
            if apic_nodes:
                ...
                for n in apic_nodes:
                    status_style = (
                        "green"
                        if n.get("health", 0) >= self.health_threshold
                        else "red"
                    )
                    apic_table.add_row(
                        str(n.get("name", "")),
        ...
        def _print_leaf_spine_table(self, leaf_spine_nodes: List[Dict]):
            ...
            if leaf_spine_nodes:
                ...
                for n in leaf_spine_nodes:
                    ...
                    cpu_style = (
                        "green" if n.get("cpu", 0) < self.cpu_mem_threshold else "red"
                    )
                    mem_style = (
                        "green"
                        if n.get("memory", 0) < self.cpu_mem_threshold
                        else "red"
                    )
        ...
        def _print_faults_table(self, faults: List[Dict]):
            ...
            if faults:
                ...
                self.console.print(
                    Panel("✓ No critical or major faults found", style="green")
                )
                self.console.print()

        def _print_error_table(
            self, errors: List[Dict], error_type: str, error_field: str
        ):
            """Print error table for specific error type"""
            if errors:
                table = Table(
                    title=f"{error_type.upper()} ERRORS (Threshold: {self.interface_threshold})",
                    box=box.ROUNDED,
                )
                ...
                for intf in errors:
                    error_style = (
                        "red"
                        if intf.get(error_field, 0) > self.interface_threshold
                        else "yellow"
                    )
                    table.add_row(
                        str(intf.get("node", "")),
                ...
                self.console.print(
                    Panel(
                        f"✓ No {error_type} errors above threshold found", style="green"
                    )
                )
                self.console.print()
        ...
        def generate_summary(
            ...
            # APIC Health
            apic_health_ok = (
                all(n.get("health", 0) >= self.health_threshold for n in apic_nodes)
                if apic_nodes
                else False
            )
            apic_count = len(apic_nodes)
            apic_problem_count = len(
                [n for n in apic_nodes if n.get("health", 0) < self.health_threshold]
            )

            # Leaf/Spine Health
            leaf_spine_health_ok = (
                all(
                    n.get("health", 0) >= self.health_threshold
                    for n in leaf_spine_nodes
                )
                if leaf_spine_nodes
                else False
            ...
            leaf_spine_health_problem_count = len(
                [
                    n
                    for n in leaf_spine_nodes
                    if n.get("health", 0) < self.health_threshold
            ...
            cpu_mem_ok = (
                all(
                    n.get("cpu", 0) < self.cpu_mem_threshold
                    and n.get("memory", 0) < self.cpu_mem_threshold
                    for n in leaf_spine_nodes
                )
                if leaf_spine_nodes
                else False
            )
            cpu_problem_count = len(
                [
                    n
                    for n in leaf_spine_nodes
                    if n.get("cpu", 0) >= self.cpu_mem_threshold
                ]
            )
            mem_problem_count = len(
                [
                    n
                    for n in leaf_spine_nodes
                    if n.get("memory", 0) >= self.cpu_mem_threshold
            ...
            # Faults
            critical_faults = len(
                [f for f in faults if f.get("severity", "").lower() == "critical"]
            )
            major_faults = len(
                [f for f in faults if f.get("severity", "").lower() == "major"]
            )
            ...
            overall_ok = (
                apic_health_ok
                and leaf_spine_health_ok
                and cpu_mem_ok
                and fabric_health_ok
                and critical_faults == 0
                and major_faults == 0
                and crc_error_count == 0
                and fcs_error_count == 0
                and drop_error_count == 0
                and output_error_count == 0
            )
            ...
            return {
                ...
                "faults": {"critical": critical_faults, "major": major_faults},
                "fcs_errors": {
        ...
        def print_summary(self, summary_data: Dict):
            ...
            # Overall status
            overall_color = (
                "green" if summary_data["overall_status"] == "PASS" else "red"
            )
            summary_text.append("OVERALL STATUS: ", style="bold")
            summary_text.append(
                f"{summary_data['overall_status']}\n", style=f"bold {overall_color}"
            )
            summary_text.append("\n")
            ...
            summary_text.append(
                f"({summary_data['apic']['problems']} of {summary_data['apic']['total']} with issues)\n"
            )

            # Leaf/Spine Status
            leaf_spine_color = (
                "green" if summary_data["leaf_spine"]["status"] == "PASS" else "red"
            )
            summary_text.append("Leaf/Spine Nodes: ", style="bold")
            summary_text.append(
                f"{summary_data['leaf_spine']['status']} ", style=leaf_spine_color
            )
            summary_text.append(
                f"({summary_data['leaf_spine']['health_problems']} health, "
            )
            summary_text.append(f"{summary_data['leaf_spine']['cpu_problems']} CPU, ")
            summary_text.append(
                f"{summary_data['leaf_spine']['mem_problems']} memory issues)\n"
            )

            # Fabric Status
            fabric_color = (
                "green" if summary_data["fabric"]["status"] == "PASS" else "red"
            )
            summary_text.append("Fabric Health: ", style="bold")
            summary_text.append(
                f"{summary_data['fabric']['status']} ", style=fabric_color
            )
            summary_text.append(f"(Score: {summary_data['fabric']['score']}%)\n")

            # Faults
            faults_total = (
                summary_data["faults"]["critical"] + summary_data["faults"]["major"]
            )
            ...
            summary_text.append(
                f"({summary_data['faults']['critical']} critical, {summary_data['faults']['major']} major)\n"
            )
            ...
            summary_text.append(
                f"CPU/Memory: {summary_data['thresholds']['cpu_mem']}%, "
            )
            summary_text.append(
                f"Interface: {summary_data['thresholds']['interface']} errors\n"
            )
            ...
            status_msg = (
                "✓ ALL CHECKS PASSED"
                if summary_data["overall_status"] == "PASS"
    ...
    class DataSaver:
        ...
        def save_report_xlsx(
            ...
            try:
                    ...
                    for sheet_name, config in sheet_configs.items():
                        if config["data"]:
                            ...
                            for item in config["data"]:
                                row = {
                                    col: str(item.get(config["key_map"][col], ""))
                            ...
                            self.console.print(
                                f"[green]✓ {sheet_name.replace('_', ' ').title()} sheet created[/green]"
                            )
                        else:
                            self.console.print(
                                f"[yellow]⚠ No data for {sheet_name} sheet[/yellow]"
                            )
                ...
                self.console.print(
                    "[red]Error: pandas and openpyxl are required for XLSX export. Install with: pip install pandas openpyxl[/red]"
                )
                return False
            except Exception as e:
